...
pair<int, Tile> checkPermutations(Tile& base, Tile& checker){
    auto temp = checkNeighbor(base, checker);
    if(temp != -1){
        return make_pair(temp, checker);
    }
    auto t = checker;
    for(int i = 0;i<3;i++){
        t = rotate90(t);
        temp = checkNeighbor(base, t);
        if(temp != -1){
    ...
    while(changed){
        ...
        for(auto& p: m){
            ...
            for(int i =0;i<inp.size();i++){
                int dir = checkNeighbor(p.second, inp[i]);
                if(dir != -1){
                    cout<<"Tiles"<<endl;
                    p.second.print();
                    cout<<"And "<<endl;
                    ...
                    switch(dir){
                        case 0: m[crd(x,y+1)] = inp[i]; inp.erase(inp.begin()+i);break;
                        case 1: m[crd(x+1,y)] = inp[i]; inp.erase(inp.begin()+i);break;
                        case 2: m[crd(x,y-1)] = inp[i]; inp.erase(inp.begin()+i);break;
                        case 3: m[crd(x-1,y)] = inp[i]; inp.erase(inp.begin()+i);break;
                    }
                    changed = true;
                    for(int x = -2;x<=2;x++){
                        for(int y = -2;y<=2;y++){
